<template>
	<view>
		<view class="wrap">
			<view class="top">
				<view class="top-list">
					...
					<view class="top-right">
						<image src="../../static/yan.png" mode="widthFix"></image>
					</view>
				</view>
				<view class="money">
					≈{{money}}
			...
			<view class="nav-top">
				<image src="../../static/bil.png" mode="widthFix"></image>
				<view class="nav-des">
					账单记录
				</view>
			</view>
		...
		methods: {
			...
			getUser(){
				...
				this.$http({
					...
					if (res.status == 'ok') {
						let x=res.data;
						let a=_this.setNum(x['e_b'])+_this.setNum(x['u_b'])+_this.setNum(x['a_b'])+_this.setNum(x['us_b']);
						_this.money=a;
			...
			setNum(number){
				let num =Math.round(number * 10000) / 10000;
				return num;
			},
			getNewList(){

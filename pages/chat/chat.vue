		...
		<view class="content">
			<scroll-view class="msg-list" scroll-y="true" :scroll-with-animation="scrollAnimation" :scroll-top="scrollTop" :scroll-into-view="scrollToView">
				<view class="row" v-for="(row,index) in msgList" :key="index" :id="'msg'+row.id">
					...
					<!-- 别人发出的消息 -->
					<view class="other" v-if="row.uid==2">
						<view class="left">
	...
	export default {
		...
		methods:{
			...
			sendMsg(content,type){
				...
				lastid++;
				let msg = {id:lastid,uid:1,username:"大黑哥123",face:"/static/img/face.jpg",time:nowDate.getHours()+":"+nowDate.getMinutes(),type:'text',msg:content,type1:1};
				console.log(msg);
				this.screenMsg(msg);
